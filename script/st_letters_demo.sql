...
--5 --------------------------------------
INSERT INTO letters (name,statement,geom) 
SELECT 'ST_ChaikinSmoothing iteration 3',

'SELECT ST_ChaikinSmoothing(geom, 3, false) 
FROM message;'
,
ST_ChaikinSmoothing(geom, 3, false) 
FROM message;
...
'SELECT 
  ...
  FROM message;'
,
ST_Union(ST_Buffer(ST_GeneratePoints(geom , 100, 1996),0.00005))
FROM message;

--15 ---------------------------------------
INSERT INTO letters (name,statement,geom) 
SELECT 'ST_GeneratePoints 600',
...
'SELECT 
  ...
  FROM message;'
,
ST_Union(ST_Buffer(ST_GeneratePoints(geom , 600, 1996),0.00005))
FROM message;
...
'SELECT 
  ST_Buffer(
    ST_LineFromMultiPoint(
     ST_GeneratePoints(geom , 600, 1996)
    )
  ,0.00005)
  FROM message;'
, 
ST_Buffer(ST_LineFromMultiPoint(ST_GeneratePoints(geom , 600, 1996)),0.00005) 
FROM message;
...
Update letters set showdate = gid + (date('2025-07-17')- xid) FROM (SELECT max(gid) as xid FROM letters) as foo;
